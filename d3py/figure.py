...
from .compat import StringIO, to_bytes, text_type
...
class Figure(object):
    ...
    def _build_html(self):
        ...
        self.html = text_type(self.template, "utf-8")
    ...
    def _build_js(self):
        ...
        draw += "    width = %s - margin.left - margin.right" % \
            self.margins["width"]
        draw += "    height = %s - margin.top - margin.bottom;" % \
            self.margins["height"]
        # this approach to laying out the graph is from Bostock:
    ...
    def _save_data(self, directory=None):
        ...
        self.filemap[filename] = {
            "fd": StringIO(to_bytes(self._data_to_json())),
            "timestamp": time.time()
        }
    ...
    def _save_html(self):
        ...
        self.html = self.html.replace("{{ port }}", text_type(self.port))
        self.html = self.html.replace("{{ host }}", text_type(self.host))
        # write html
        filename = "%s.html" % self.name
        self.filemap[filename] = {
            "fd": StringIO(to_bytes(self.html)),
            "timestamp": time.time()
        }
    ...
    def show(self, interactive=None):
        ...
        if blocking:
            ...
            webbrowser.open_new_tab(
                "http://%s:%s/%s.html" % (self.host, self.port, self.name))
    ...
    def _serve(self, blocking=True):
        ...
        if self._server_thread is None or \
                self._server_thread.active_count() == 0:
